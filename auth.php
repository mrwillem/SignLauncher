...
function with_data_lock(callable $operation): mixed
{
    ...
    if ($lock === false || !flock($lock, LOCK_EX)) {
        throw new RuntimeException('Datenspeicher ist nicht verfügbar.');
    }
    try {
        return $operation();
    } finally {
...
{
    return read_json_file(app_path('auth.json'));
}
function save_auth_data(array $data): void
{
    write_json_file(app_path('auth.json'), $data);
}
function is_configured(): bool
{
    return isset(auth_data()['password_hash']);
}
...
function display_id_valid(string $id): bool
{
    return (bool) preg_match('/^[A-Za-z0-9_-]+$/', $id);
}
function default_displays(): array
{
    return [['id' => 'eisbar', 'name' => 'Eisbar', 'orientation' => 0], ['id' => 'theke', 'name' => 'Theke', 'orientation' => 0], ['id' => 'food', 'name' => 'Food', 'orientation' => 0], ['id' => 'eingang', 'name' => 'Eingang', 'orientation' => 0], ['id' => 'stehle', 'name' => 'Stehle', 'orientation' => 0]];
}
function legacy_yaml_displays(string $path): array
{
    ...
    foreach (file($path, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [] as $line) {
        ...
        } elseif ($current !== null && preg_match('/^\s+name:\s*(.+)$/', $line, $m)) {
            $value = trim($m[1]);
            $decoded = json_decode($value, true);
            $current['name'] = is_string($decoded) ? $decoded : trim($value, " \t\"'");
        } elseif ($current !== null && preg_match('/^\s+orientation:\s*(\d+)\s*$/', $line, $m)) {
            $current['orientation'] = (int) $m[1];
        }
    }
    if ($current !== null) {
        $result[] = $current;
...
{
    ensure_data_dir();
    $path = app_path('displays.json');
    if (!is_file($path)) {
        $legacy = app_path('displays.yaml');
        $migrated = is_file($legacy) ? legacy_yaml_displays($legacy) : default_displays();
        save_displays($migrated);
        if (is_file($legacy)) {
            unlink($legacy);
        }
    }
...
function save_displays(array $displays): void
{
    write_json_file(app_path('displays.json'), $displays);
}
function display_by_id(string $id): ?array
{
...
function valid_screen(string $screen): bool
{
    return display_by_id($screen) !== null;
}
...
function record_failed_login(array &$data): void
{
    ...
    $attempts = $data['login_attempts'][$key] ?? [];
...
function clear_failed_logins(array &$data): void
{
    unset($data['login_attempts'][login_attempt_key()]);
}

function events(): array
{
    return read_json_file(app_path('events.json'));
}
function save_events(array $events): void
{
    write_json_file(app_path('events.json'), $events);
}
function media_path(string $file): string
{
    return app_path('media/' . basename($file));
}
